...
def log(message: Optional[str] = None, open_log: bool = False) -> None:
    oswandlog = os.getenv("WAND_LOG") or os.getenv("WEMOD_LOG")
    wandlog = oswandlog
    cowandlog = load_conf_setting("WandLog") or load_conf_setting("WeModLog")
    if not wandlog:
        wandlog = cowandlog
    if wandlog != "":
        try:
    ...
    if os.path.isfile(responsefile):
        ...
        if bout != None:
            batresp = show_message(
                returnmessage
                + f'\nYou can still use Wand by clicking "Yes",\nthis will keep Wand open in the background\nIf you want to close Wand click "No"\nWand will automatically close in {bout} seconds, if nothing is done',
                "BAT Warning",
                bout,
                True,
            )
