...
def getbatcmd():
    ...
    # Migrate old wemod.bat if wand.bat is missing
    old_batf = os.path.join(SCRIPT_PATH, "wemod.bat")
    if not os.path.isfile(batf) and os.path.isfile(old_batf):
        import shutil
        shutil.copy2(old_batf, batf)
